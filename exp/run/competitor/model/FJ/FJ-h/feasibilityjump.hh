...
u_int64_t UINT_MAX = std::numeric_limits<u_int64_t>::max();
long INT_MIN = -std::numeric_limits<int>::max();
...
public:
    ...
    int solve(std::chrono::_V2::system_clock::time_point startTime, double *initialValues, std::function<CallbackControlFlow(FJStatus)> callback)
    {
        assert(callback);
        bool foundFeas = false;
        if (verbosity >= 1)

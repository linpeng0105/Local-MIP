    ...
    const char *_filename)
{
  ...
  while (getline(infile, readLine)) // BOUNDS section
  {
    ...
    if (var.type == MIP_VarType::Binary)
    {
      var.SetType(MIP_VarType::Integer);
    ...
    {
      var.SetType(MIP_VarType::Binary);
...
bool ReaderMPS::TightBoundGlobally()
{
  for (auto &modelVar : modelVarUtil->varSet)
    if (modelVar.IsFixed())
    {
      modelVar.SetType(MIP_VarType::Fixed);
    ...
    for (size_t termIdx = 0; termIdx < removeVar.conIdxSet.size(); termIdx++)
    {
      ...
      else
      {
        ...
        if (modelCon.varIdxSet.size() == 1)
        {
          ...
          if (relatedVar.type != MIP_VarType::Fixed &&
              relatedVar.IsFixed())
          {
            relatedVar.SetType(MIP_VarType::Fixed);
...
bool ReaderMPS::SetVarType()
{
  for (size_t varIdx = 0; varIdx < modelVarUtil->varNum; varIdx++)
  {
    ...
    else if (modelVar.IsBinary())
    {
      ...
      modelVar.SetType(MIP_VarType::Binary);
    }
    else if (modelVar.type == MIP_VarType::Integer)
    {
      modelVarUtil->integerNum++;
    }
    else
    {
      modelVar.SetType(MIP_VarType::Real);

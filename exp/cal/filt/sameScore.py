    ...
    def cal_soln(self, ins_name):
        if (not ins_name in self.datas):
            ...
            try:
                ...
                if (not len(re.findall(r"s .*", fstr)) == 0):
                    best_objstr = re.findall(r"s .*", fstr)[-1]
                    self.sameScore+=float(best_objstr.split()[3])
                    # print(best_objstr, best_objstr[3])
            except:
                pass
...
class calculater(object):
    ...
    def cal_and_show(self, show=True):
        ...
        for sample in self.sample_dirs:
            ...
            print(f"cc {self.solvers[0].sameScore/495}")
                # if True:
                # # if False:
                #    print(ins_detail)
                #    print()\
...
if __name__ == "__main__":
    ...
    solvers.append(solver(
        f"/pub/netdisk1/linpeng/Local-MIP/result-new/Local-MIP/v6/ablation/sameScore/3600", "Local-MIP"))
    samples = []
    samples.append(

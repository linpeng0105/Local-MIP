    ...
    def cal_and_show(self, show=True):
        ...
        for idx in range(0, len(self.sample_dirs)):
            ...
            for ins_name in open(samp_dir):
                ...
                A = self.solvers[0].datas[ins_name].best_obj
                B = self.solvers[1].datas[ins_name].best_obj
                if A > B:
                    better += 1
                if A < B:
                    worse += 1
            ...
            if better > worse:
                wins[idx] = f"{wins[idx]} &\\textbf{{{better}}} &{worse}"
            if better < worse:
                wins[idx] = f"{wins[idx]} &{better} &\\textbf{{{worse}}}"

...
class solver(solver):
    def cal_soln(self, ins_name):
        if (not ins_name in self.datas):
            ...
            try:
                obj_time = []
                for line in open(f"{real_file_path}", "r"):
                    line = line.split()
                    obj_time.append([float(line[0]), float(line[1])])
                best_obj = obj_time[0][1]
                self.datas[ins_name].best_obj = best_obj
            except:
                self.datas[ins_name].best_obj = 300
                pass
        ...
        for idx in range(0, len(self.sample_dirs)):
            ...
            for ins_name in open(samp_dir):
                ...
                for slv in self.solvers:
                    ...
                    slv.win_num += slv.datas[ins_name].best_obj
                ...
                # if self.solvers[0].datas[ins_name].best_obj ==0:
                #     print(f"{ins_name}")
                print(f"[{self.solvers[0].datas[ins_name].best_obj},{self.solvers[1].datas[ins_name].best_obj}],")
            best_win = 300
            for slv in self.solvers:
                slv.win_num /= insnum[idx]
                best_win = min(best_win, slv.win_num)
            for slv in self.solvers:
